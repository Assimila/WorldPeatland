...
def main(ts_path):
    ...
    for path in (glob(f'{ts_path}/*.tif')):
        ...
        for i, (year, _time) in enumerate(grouped_by_year):
            ...
            LOG.info(ds.time.to_index().duplicated().any())  # True if there are duplicates
            _, index = np.unique(ds.time, return_index=True)
            ds_unique = ds.isel(time=index)
            ds_year = ds_unique.sel(time=_time)
        ...
        fname = filename.replace('.tif', '_climatology_mean.tif')
        output_path_mean = os.path.join(output_dir, fname)
        fname = filename.replace('.tif', '_climatology_std.tif')
        output_path_std = os.path.join(output_dir, fname)
        fname = filename.replace('.tif', '_anomalies.tif')
        output_path_anomalies = os.path.join(output_dir, fname)

        # edited version of save_xarray_to_gtiff
        save_tiff(output_path_mean, ds_mean, var_name)
        LOG.info(f'{output_path_mean} successfully saved')
        save_tiff(output_path_std, ds_std, var_name)
        LOG.info(f'{output_path_std} successfully saved')
        save_tiff(output_path_anomalies, ds_anomalies_all_years, var_name)
        LOG.info(f'{output_path_anomalies} successfully saved')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        print("Usage: python script.py <cross_ratio_ts_path>")
    else:
