...
def run_cprob(outputs, product='S2_TOA'):
    ...
    output_dir = os.path.join(OUTPUTDIR, 'datacube', product, 'cprob')
    create_dir(output_dir)
...
def create_monthly_cogs(ouputs_cprob, year, month, product = 'S2_TOA'):
    """
    Create monthly DataCube COGs
    """
    
    for dataset in outputs_cprob:
        ...
        vrt = gdal.BuildVRT(output_vrt_fname, outputs_cprob[dataset],
                options=build_options)

        for i in range(vrt.RasterCount):
            _time = outputs_cprob[dataset][i]
        ...
        output_dir = Path(outputs_cprob[dataset][0])
        ...
        for f in outputs_cprob[dataset]:
            os.remove(f)
...
for year in range(2017, 2024+1):
    for month in range(1, 12+1):
        ...
        if not outputs :
            ...
            outputs_cprob = run_cprob(outputs, product='S2_TOA')
