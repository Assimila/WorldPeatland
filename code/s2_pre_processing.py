...
def main(S2_path):
    ...
    for month_tif in B02_tif_files_list:
        ...
        filename = os.path.basename(month_tif)
        ...
        # Empty lists to store the biophysical MLEONN products
        MLEONN_lai_list = []
        MLEONN_fapar_list = []
        MLEONN_fc_list =[]
        MLEONN_cab_list = []
        ...
        for bd in range(opn.RasterCount):
            ...
            arr = (arr * 0.0001).astype(np.float32)
            ...
            for tif in reflectance_tifs:
                ...
                input_dict[f'{band_name}'] = (arr_g * 0.0001).astype(np.float32)
            ...
            # Append the MLEONN product arrays to the list
            MLEONN_lai_list.append(lai)
            MLEONN_fapar_list.append(fapar)
            MLEONN_fc_list.append(fc)
            MLEONN_cab_list.append(cab)

        # MLEONN products name list
        MLEONN_products = ['lai', 'fapar', 'fc', 'cab']

        for product in MLEONN_products:

            # loop over all MLEONN generated products to save them to cog monthly tiffs

            # stack the numpy arrays in the lists
            stacked_arr = np.stack(eval(f'MLEONN_{product}_list'), axis=0)
