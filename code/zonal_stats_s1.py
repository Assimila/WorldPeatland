...
def main(data_tif_path, shapefile_path):
    # get tif file name
    ...
    directory = os.path.dirname(data_tif_path)

    # save the raw zonal stat in a pickle file
    output_path = create_dir(directory, 'ZonalStats')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        ...
        print("Usage: python script.py <data_tif_path> <shapefile_path>")  # the user has to input one argument
    else:
        data_tif_path = sys.argv[1]
        shapefile_path = sys.argv[2]
        main(data_tif_path, shapefile_path)
